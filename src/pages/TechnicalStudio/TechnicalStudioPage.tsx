...
export function TechnicalStudioPage({ garmentId, onOpenGarment }: { garmentId?: string; onOpenGarment: (garmentId: string) => void }) {
  ...
  if (garmentId) return <CanonicalWorkspaceState><SpecialistWorkbench><GarmentWorkbenchContext garmentId={garmentId} label="Technical Studio" /><WorkbenchTabs active={technicalSection} ariaLabel="Technical Studio sections" items={technicalWorkbenchTabs} onChange={setTechnicalSection} />{technicalSection === 'flats' ? <FlatsWorkspace garmentId={garmentId} /> : technicalSection === 'pom' || technicalSection === 'measurements' ? <MeasurementStudio garmentId={garmentId} onOpenFlats={() => setTechnicalSection('flats')} section={technicalSection} /> : <ReleaseStudio garmentId={garmentId} section={technicalSection} />}</SpecialistWorkbench></CanonicalWorkspaceState>;
