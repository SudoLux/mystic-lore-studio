...
-- A concise, canonical detail field for calendar appointments. This is not a
-- separate notes table: it belongs to the existing private event record.
alter table ml_private.calendar_events
  add column if not exists notes text not null default '';

-- The browser transport is an explicit allowlist. Adding a SQL column alone
-- must never make it client writable, so the existing function is updated only
-- for the new event detail field.
...
comment on column ml_private.calendar_events.notes is 'Private calendar appointment detail, not a separate task or event record.';
