...
function validateSingleFlat(state: CanonicalWorkspaceState, flat: CanonicalTechnicalFlat) { const asset = state.mediaAssets.find((item) => item.id === flat.assetId); const file = state.technicalFiles.find((item) => item.specId === flat.specId && item.assetId === flat.assetId && item.isSource); if (!asset?.localBlobKey || !asset.checksum || !file) return [{ message: 'Store and map the source file before approval.' }]; if (state.flatAnnotations.some((item) => item.flatId === flat.id && item.severity === 'critical' && item.status === 'open')) return [{ message: 'Resolve critical annotations before approval.' }]; return []; }
