...
function validateAnchor(anchor: { x: number; y: number }) { if (![anchor.x, anchor.y].every((value) => Number.isFinite(value) && value >= 0 && value <= 1)) throw new Error('POM anchors must be normalized from 0 through 1.'); }
