...
const grading = readFileSync(new URL('../src/pages/TechnicalStudio/GradingWorkspace.tsx', import.meta.url), 'utf8');
...
describe('WP4 POM and measurement UI contracts', () => {
  ...
  it('previews grading before creating a separate measurement set', () => { expect(grading).toContain('Live preview'); expect(grading).toContain('Create Graded Set'); expect(grading).toContain('base set remains unchanged'); });
