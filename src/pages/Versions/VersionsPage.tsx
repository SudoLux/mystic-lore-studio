  ...
  return <SpecialistWorkbench>
    ...
    <div className="grid min-w-0 gap-5 xl:grid-cols-[18rem_minmax(0,1fr)]">
      ...
      <main className="min-w-0 space-y-5">
        ...
        <Card><div className="mb-5 flex flex-wrap items-start justify-between gap-3"><div><p className="text-xs uppercase tracking-[0.14em] text-ember">Structural comparison</p><h2 className="font-display mt-2 text-2xl">Field and row evidence</h2></div><div className="flex gap-2"><Button disabled={!diffs.length} icon={<Download size={15} />} onClick={exportDiff} size="sm">Export diff</Button><Button disabled={!selectedKeys.length || busy || studio.conflicts.length > 0 || targetId !== 'working'} icon={<RotateCcw size={15} />} onClick={() => void prepareRestore()} size="sm" variant="primary">Preview restore</Button></div></div><DiffViewer diffs={diffs} selectedKeys={selectedKeys} setSelectedKeys={setSelectedKeys} /></Card>
