...
function RecentGarmentCard({ garment, onOpen, state }: { garment: CanonicalGarment; onOpen: () => void; state: CanonicalWorkspaceState }) {
  ...
  const collectionName = collection?.name ?? 'Independent piece';

  return <article className="overflow-hidden rounded-[1.25rem] bg-stardust/[0.025]">
    <button aria-label={`Open ${garment.title}, ${collectionName}, ${garment.phase} garment`} className="group block w-full overflow-hidden rounded-[1.25rem] text-left transition duration-300 hover:-translate-y-1 hover:bg-stardust/[0.045] active:translate-y-0 active:scale-[0.992] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ember focus-visible:ring-offset-2 focus-visible:ring-offset-midnight motion-reduce:transform-none motion-reduce:transition-none" onClick={onOpen} type="button">
      <CanonicalMediaImage alt={`${garment.title} garment`} asset={cover} className="aspect-[4/5] w-full rounded-b-none transition duration-500 group-hover:scale-[1.025] group-focus-visible:scale-[1.025] motion-reduce:transform-none motion-reduce:transition-none" derivatives={state.mediaDerivatives} fit="cover" mode="library" />
      <div className="p-4"><div className="flex items-start justify-between gap-3"><div className="min-w-0"><h3 className="truncate text-base font-semibold text-stardust">{garment.title}</h3><p className="mt-1 truncate text-xs text-stardust/44">{collectionName}</p></div><PhasePill phase={garment.phase} /></div><span aria-hidden="true" className="mt-4 inline-flex items-center gap-2 text-sm font-medium text-ember transition-transform duration-200 group-hover:translate-x-0.5 group-focus-visible:translate-x-0.5 motion-reduce:transform-none">Open garment <ArrowRight size={15} strokeWidth={2.25} /></span></div>
    </button>
  </article>;
}
